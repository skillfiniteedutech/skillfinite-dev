...
export function AnimatedTestimonials({
  ...
  return (
    <section ref={sectionRef} id="testimonials-animated" className={`py-24 overflow-hidden bg-orange-50/50 ${className || ""}`}>
        ...
        >
            ...
            <div className="space-y-6">
              {badgeText && (
                <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-orange-100 text-orange-600">
                  <Star className="mr-1 h-3.5 w-3.5 fill-orange-600" />
                  <span>{badgeText}</span>
                </div>
              )}
              ...
              <div className="flex items-center gap-3 pt-4">
                {testimonials.map((_, index) => (
                  <button
                    ...
                    className={`h-2.5 rounded-full transition-all duration-300 ${activeIndex === index ? "w-10 bg-orange-500" : "w-2.5 bg-gray-300"
                      }`}
          ...
          <motion.div variants={itemVariants} className="relative h-full mr-10 min-h-[300px] md:min-h-[400px]">
            {testimonials.map((testimonial, index) => (
              ...
              >
                <div className="bg-white border border-gray-100 shadow-xl rounded-xl p-8 h-full flex flex-col">
                  ...
                  <div className="relative mb-6 flex-1">
                    <Quote className="absolute -top-2 -left-2 h-8 w-8 text-orange-500/20 rotate-180" />
            ...
            <div className="absolute -top-6 -right-6 h-24 w-24 rounded-xl bg-orange-500/5"></div>
          </motion.div>
        </motion.div>
          ...
          <motion.div variants={itemVariants} initial="hidden" animate={controls} className="mt-24 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-x-12 gap-y-8">
              {trustedCompanies.map((company) => (
                <div key={company} className="text-xl font-bold text-gray-300 hover:text-gray-400 transition-colors cursor-default">
                  {company}
                </div>
              ))}
            </div>
